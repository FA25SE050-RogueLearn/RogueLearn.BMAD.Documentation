...
CREATE TYPE skill_reward_source_type AS ENUM ('Class', 'Quest', 'Event', 'Other', 'AcademicRecord');
...
#### subjects
Master list of all subjects. Each record is unique for a given code and version combination.
```sql
CREATE TABLE subjects (
    ...
    content JSONB, -- Stores the full syllabus content as JSON
    semester INTEGER NOT NULL, -- The official semester this subject belongs to in a curriculum
    prerequisite_subject_ids UUID[], -- Array of subject UUIDs that are prerequisites
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
#### student_enrollments
Tracks a student's high-level enrollment period at the institution.
```sql
...
The student's "gradebook", tracking which subjects were taken in which semester and the outcome.
```sql
CREATE TABLE student_semester_subjects (
    ...
    learned_at_semester INTEGER NOT NULL, -- The actual semester the student took the subject
...
Admin-curated many-to-many relationship between subjects and skills, with a relevance weight.
```sql
...
CREATE TABLE user_skill_rewards (
    ...
    source_type skill_reward_source_type NOT NULL, -- e.g., QuestComplete, BossFight, AcademicRecord
